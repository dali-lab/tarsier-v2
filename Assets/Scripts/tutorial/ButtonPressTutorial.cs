...
public class ButtonPressTutorial : TutorialBaseClass
{
    ...
    public override void Update()
    {
        if (currentIndex == 1)           // on button screen
        {
            ...
            if (_inputManager.IsButtonPressed(Buttons.BUTTON_B))
            {
                R2.SetActive(false);                 // turn off glow on R button 2
               
            }
            if (_inputManager.IsButtonPressed(Buttons.BUTTON_X))
            {
                L1.SetActive(false);                 // turn off glow on L button 1
            }
            if (_inputManager.IsButtonPressed(Buttons.BUTTON_Y))
            {
        ...
        } else if (currentIndex == 2)           // on trigger screen
        {
            ...
            if (_inputManager.IsButtonPressed(Buttons.RIGHT_TRIGGER))
            {
                RTrigger.SetActive(false);                 // turn off glow on R trigger
            }
            if (_inputManager.IsButtonPressed(Buttons.LEFT_TRIGGER))
            {
        ...
        } else if (currentIndex == 3)       // on grip screen
        {
            ...
            if (_inputManager.IsButtonPressed(Buttons.RIGHT_GRIP))
            {
                RGrip.SetActive(false);                 // turn off glow on R grip
            }
            if (_inputManager.IsButtonPressed(Buttons.LEFT_GRIP))
            {
